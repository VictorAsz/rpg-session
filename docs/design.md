...
Context:
- Angular 21
- RxJS Store pattern (BehaviorSubject + selectors)
- Domain-driven structure (service + store per domain)
- Supabase only for persistence + realtime
- Frontend contains all business logic

We already defined:
- PostgreSQL schema (characters, dice_rolls, items, buffs, etc.)
- TypeScript interfaces
- Stores for each domain

Now:

You are a real-time systems expert.

Implement a real-time sync layer using Supabase Realtime for an Angular app.

Requirements:
- Listen to changes in:
  - Characters
  - Rolls
  - Items usage/ item equip
  - etc..
...
- Avoid memory leaks
- Must be scalable and clean

Deliver:
1. Angular service for realtime subscriptions
2. Example subscription for character updates
3. Example subscription for dice rolls
4. Strategy to merge realtime updates into local state
5. How to avoid race conditions

Important:
Keep frontend as source of logic, Supabase only syncs data.